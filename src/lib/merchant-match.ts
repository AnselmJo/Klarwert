...
/**
 * Normalisiert einen Empfänger-String:
 * - Kleinschreibung & Trimming
 * - Entfernt Mehrfach-Leerzeichen
 * - Entfernt typische Referenznummern, Filial-Codes, Ort/PLZ-Zusätze und Terminal-IDs am Ende
 */
export function normalizeCounterparty(text: string): string {
  ...
  // Entferne Suffix-Muster: Filialnummern (z. B. "sagt danke 1234", "filiale 456", "nr. 789", "#123")
  s = s.replace(/\b(filiale|nr|store|shop|kasse|station)\s*#?\d+\b/gi, "");
  s = s.replace(/\b(sagt danke|danke|vielen dank)\s*\d*\b/gi, "");

  // Entferne Hex/Zahlen-Codes am Ende (z.B. "rewe markt 3840", "amazon.de*mb1234567")
  s = s.replace(/[\s*-]+[a-z0-9]{5,20}$/i, "");
  s = s.replace(/\b\d{4,8}\b/g, "");

  // Säubere doppelte Leerzeichen und Satzzeichen am Ende
...
export function extractMerchantFromPaymentProvider(tx: {
  ...
  if (cpLower.includes("paypal") || cpLower.startsWith("pp.")) {
    ...
    // Fallback: Zweck durchsuchen
    if (purpose) {
      // Muster: "Ihr Einkauf bei <Merchant>", "Shop: <Merchant>", "Einkauf <Merchant>", "Bestellung bei <Merchant>"
      const einkaufMatch = purpose.match(/(?:einkauf bei|bestellung bei|shop:|verkaeufer:|händler:)\s*([^,.\n]+)/i);
      if (einkaufMatch && einkaufMatch[1].trim().length > 1) {
        return { merchantName: einkaufMatch[1].trim(), extractedFrom: "purpose" };
      }
      // Erstes Wort/Satz im Zweck vor Referenzcode
      const firstToken = purpose.split(/[,;\n]/)[0].replace(/^(paypal|ihr einkauf|rechnung)\s*/i, "").trim();
      if (firstToken.length > 2 && !/^\d+$/.test(firstToken) && !PAYMENT_PROVIDER_BLOCKLIST.has(firstToken.toLowerCase())) {
        return { merchantName: firstToken, extractedFrom: "purpose" };
      }
    }
  }
  ...
  if (cpLower.includes("klarna")) {
    ...
    if (purpose) {
      const shopMatch = purpose.match(/(?:shop|händler|verkäufer|bei)\s*[:\s]\s*([^,.\n]+)/i);
      if (shopMatch && shopMatch[1].trim().length > 1) {
        return { merchantName: shopMatch[1].trim(), extractedFrom: "purpose" };
      }
    }
  }
