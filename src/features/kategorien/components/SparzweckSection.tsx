...
export function SparzweckSection() {
  ...
  function invalidate() {
    queryClient.invalidateQueries({ queryKey: ["sparzwecke"] });
    queryClient.invalidateQueries({ queryKey: ["sparzwecke-totals-page"] });
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <h2 className="font-heading text-lg text-charcoal">Sparzwecke</h2>
        <Button
          size="sm"
          variant="ghost"
          onClick={() => {
      ...
      <div className="rounded-standard border border-border bg-card">
        {sparzwecke?.map((s) => (
          <div key={s.id} className="flex items-center gap-3 border-b border-border p-2 last:border-0">
            ...
            <Button size="icon" variant="ghost" aria-label="Löschen" onClick={() => setDeleteTarget(s)}>
              <Trash2 className="size-4" />
            </Button>
          </div>
        ))}
        {(!sparzwecke || sparzwecke.length === 0) && (
          <p className="p-3 text-sm text-slate">Noch keine Sparzwecke.</p>
        )}
      </div>
      ...
      {deleteTarget && (
        <ConfirmDialog
          ...
          description="Entfernt nur die Zuordnung – das Sparen-Flag bestehender Transaktionen bleibt erhalten."
