...
export function CustomFieldsManager() {
  ...
  async function handleDelete(id: number) {
    if (!confirm("Feld wirklich löschen? Alle zugehörigen Daten gehen verloren.")) return;
  ...
  if (!customFields || customFields.length === 0) {
    return (
      <p className="text-sm text-slate">
        Noch keine benutzerdefinierten Felder. Felder werden beim Import von Spalten angelegt, die du als „Als Extra-Feld importieren" markierst.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-2">
          ...
          {editId === field.id ? (
            ...
            <>
              <div className="flex-1">
                ...
                <div className="text-xs text-slate">Typ: {DATA_TYPE_LABELS[field.data_type] ?? field.data_type}</div>
              </div>
      ...
      <p className="text-xs text-slate">
        Neue Felder werden automatisch angelegt, wenn du beim Datei-Import Spalten als „Als Extra-Feld importieren" markierst.
      </p>
    </div>
  );
}
