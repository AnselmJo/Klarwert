...
function categoryLabel(tx: SteuerTransaction): string {
  if (!tx.categoryName) return "Unkategorisiert";
  return tx.parentCategoryName ? `${tx.parentCategoryName} · ${tx.categoryName}` : tx.categoryName;
}

function exportTransactions(filename: string, transactions: SteuerTransaction[]) {
  const headers = [
    "Datum",
    "Konto",
    "Empfänger",
      ...
      <div className="flex items-center justify-between gap-3 p-4">
        <button type="button" className="flex min-w-0 flex-1 items-center gap-2 text-left" onClick={onToggle}>
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate">
              {transactions.length} Buchungen · {formatEur(sum)}
            </p>
          </div>
        </button>
        <div className="flex gap-1">
          <Button
            variant="ghost"
            size="icon"
            aria-label={`${thema.name} als CSV exportieren`}
            onClick={() => exportTransactions(`steuer-${thema.name.toLowerCase().replace(/\s+/g, "-")}.csv`, transactions)}
          >
            <Download className="size-4" />
          </Button>
          <Button variant="ghost" size="icon" aria-label={`${thema.name} bearbeiten`} onClick={onEdit}>
      ...
      {expanded && (
        <div className="border-t border-border px-4 pb-4">
          {transactions.length === 0 ? (
            <p className="pt-4 text-sm text-slate">Keine Buchungen in diesem Jahr.</p>
          ) : (
              ...
              <table className="w-full min-w-[760px] text-sm">
                <thead className="text-xs text-slate">
                  <tr className="border-b border-border">
                    ...
                    <th className="py-2 text-left font-medium">Empfänger</th>
                    <th className="py-2 text-left font-medium">Kategorie</th>
                    <th className="py-2 text-left font-medium">Vertrag</th>
                ...
                <tbody>
                  {transactions.map((tx) => (
                    ...
                    >
                      ...
                      <td className="py-2 text-slate">{categoryLabel(tx)}</td>
                      <td className="py-2 text-slate">
                        {tx.contractName ? (
                          <>
                            {tx.contractName}
                            <div className="text-xs">
                              Jahr: {formatEur(tx.contractYearSumCents ?? 0)}
                            </div>
                          </>
                        ) : (
                          "–"
...
export function SteuerPage() {
  ...
  const { data: topicTransactions } = useQuery({
    ...
    queryFn: async () => {
      ...
      return Object.fromEntries(entries) as Record<number, SteuerTransaction[]>;
    },
    enabled: !!themen,
  });
  ...
  function invalidate() {
    queryClient.invalidateQueries({ queryKey: ["steuer-themen"] });
    queryClient.invalidateQueries({ queryKey: ["steuer-topic-transactions"] });
    queryClient.invalidateQueries({ queryKey: ["steuer-search"] });
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-xl text-charcoal">Steuer</h1>
          <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate">
            <span>{year}</span>
            <span aria-hidden="true">·</span>
            <span>{themen?.length ?? 0} Themen</span>
            <span aria-hidden="true">·</span>
        ...
        <div className="flex flex-wrap gap-2">
          <Select value={String(year)} onValueChange={(value) => setYear(Number(value))}>
            <SelectTrigger className="w-[120px]" aria-label="Steuerjahr">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Button
            variant="ghost"
            onClick={() => exportTransactions(`steuer-${year}-gesamt.csv`, searchResults ?? [])}
          >
            <Download className="size-4" />
            Gesamtjahr CSV
          </Button>
      ...
      <div className="relative max-w-lg">
        <Input
          ...
          placeholder="Transaktionen im Steuerjahr durchsuchen…"
        />
      </div>
      ...
      <div className="space-y-3">
        ...
        {(!themen || themen.length === 0) && (
          <div className="rounded-standard border border-border bg-card p-8 text-center">
            ...
            <h2 className="mt-3 font-heading text-lg text-charcoal">Noch keine Steuer-Themen</h2>
            <p className="mx-auto mt-1 max-w-md text-sm text-slate">
              Lege Themen an, um relevante Buchungen für die Steuererklärung zu sammeln.
            </p>
          </div>
        )}
      </div>
      ...
      <section className="rounded-standard border border-border bg-card p-4">
        <div className="mb-3 flex items-center justify-between gap-3">
          <h2 className="text-sm font-semibold text-charcoal">Suchtreffer im Jahr</h2>
          <span className="text-xs text-slate">
            {searchResults?.length ?? 0} Buchungen · {formatEur(searchTotal)}
          </span>
        </div>
