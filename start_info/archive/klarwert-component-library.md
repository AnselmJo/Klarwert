...
**Ergebnis der Konsolidierung:** Der Prototyp verwendet **~48 einzelne CSS-Klassen/Ad-hoc-Styles** für UI-Bausteine. Diese werden hier auf **34 Komponenten** (in 4 Gruppen) plus **2 fundamentale Systeme** (Icon-System, Popover-Primitive) konsolidiert. Kapitel 3 zeigt die vollständige Zuordnung alt → neu.

**Nachträge:** B13 „Sortierbare Liste" (Regel-Prioritäts-Verwaltung, Drag&Drop + Pfeil-Buttons) und B3b „Spalten-Auswahl" (optionale Bankfelder in der Transaktionstabelle) wurden nach Praxis-Feedback ergänzt.
...
| Varianten | `text`, `zahl`, `datum` (Text + Kalender-Popover-Trigger), `auswahl` (Select), `suche` (Text mit führendem Such-Icon + Clear-Icon bei Eingabe) |
...
| Icons | `search` (führend, nur Suche-Variante) · `x` (folgend, nur wenn Suche-Variante befüllt ist, zum Leeren) · `calendar` (folgend, nur Datum-Variante) · `chevron-down` (folgend, nur Auswahl-Variante) |
| Verhalten | Frei editierbar; Auswahl-Variante öffnet native/angepasste Optionsliste; Such-Variante filtert i. d. R. live (kein expliziter Such-Button) |
...
| Feld | Beschreibung |
|---|---|
| Zweck | Ein-/Ausblenden optionaler Tabellenspalten, ohne Daten zu verlieren – primärer Anwendungsfall: Transaktionstabelle mit optionalen Bankfeldern (Tags, IBAN, Transaktionstyp etc., siehe Product Spec 4.3) |
| Varianten | Nur eine Variante |
| Größen | Icon-Button `sm` (siehe A1) + Popover-Liste, Breite ~240px |
| States | Icon-Button: Standard/Hover/Fokus; je Spalte in der Liste: aus/ein (Checkbox) |
| Icons | `eye` (Auslöser-Icon, unabhängig vom aktuellen Zustand – kein Wechsel zu `eye-off`, da mehrere Spalten gleichzeitig unterschiedliche Sichtbarkeit haben können) |
| Verhalten | Klick öffnet Popover mit einer Checkbox je optionaler Spalte; Toggle wirkt sofort auf die Tabelle; Auswahl wird als lokale UI-Präferenz gespeichert (persistiert über Sitzungen hinweg, geräteweit, kein Bezug zu einzelnen Transaktionen) |
| Keyboard Navigation | Öffnen per `Enter`/`Leerzeichen` auf dem Icon-Button; innerhalb der Liste `Pfeil hoch/runter` zwischen Checkboxen, `Leerzeichen` togglet; `Esc` schließt |
| Accessibility | Icon-Button `aria-label="Spalten ein-/ausblenden"`; Popover `role="menu"`, jede Checkbox mit sprechendem Label (voller Spaltenname, nicht nur Icon) |
...
| Varianten | `Standard` (Icon + Label) · `Gruppenüberschrift` (nur Text, nicht klickbar, rein strukturierend) · `Profil` (zusätzlich ein Avatar-Punkt links und ein Einstellungen-Icon rechts, für den Haushalts-Eintrag unten in der Sidebar) |
...
| C1 Modal, C2 Drawer | shadcn `dialog` bzw. `sheet` (side="right", Breiten 390/430px) |
| C3 Toast | shadcn `sonner` (3 s, Undo-Action, kein Stacking: `toast.dismiss()` vor neuem Toast) |
| B10 Disclosure | shadcn `collapsible` |
| B13 Sortierbare Liste | `@dnd-kit/sortable` + Pfeil-Buttons (A1 icon-only) |
