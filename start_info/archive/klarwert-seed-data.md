...
Header-Fingerprint = normalisierte Header-Zeile (lowercase, Umlaute vereinfacht, Spaltennamen sortiert verkettet). **Hinweis für die Implementierung:** Die folgenden Header entsprechen den gängigen Export-Formaten (Stand 2025/2026), müssen aber beim ersten realen Test je Bank gegen eine echte Datei verifiziert werden – Banken ändern Formate gelegentlich. Der Erkennungsmechanismus (Fingerprint-Match → sonst Heuristik → sonst manuell) fängt Abweichungen ab.
...
| DKB | `,` | utf-8 | dd.MM.yy | de | Buchungsdatum→date · Betrag (€)→amount · Zahlungsempfänger*in→counterparty (bei Ausgang) · Verwendungszweck→purpose · Kundenreferenz→external_id |
